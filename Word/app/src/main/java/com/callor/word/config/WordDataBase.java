...
// 자동 생성하면 사용하지 않는 클래스도 자동 생성되므로
// 추상화로 가져오기?

/**
 *  RoomDatabase를 상속받아 DB Connection, Session을 만드는 클래스
 *  이 클래스는 반드시 abstract 키워드를 추가하여 추상클래스로 생성하라.
 *
 *  abstract란
 *  일부는 직접 코드를 구현하는 method를 포함하고
 *  일부는 interface처럼 코드가 구현되지 않은 method를 함께
 *  포함하는 클래스이다.
 */
...
/*
    ...
    version 항목의 내용
    혹시 사용과정에서 table에 변경사항이 발생할 수 있음
...
public abstract class WordDataBase extends RoomDatabase {
    ...
    public static WordDataBase getDataBase(final Context context) {
        if(dbConn == null) {
            //
            dbConn = Room.databaseBuilder(//테이블을 만들기 전 저장소 역할, "word database"라는 데이터 저장소를 만들기 위한 코드
